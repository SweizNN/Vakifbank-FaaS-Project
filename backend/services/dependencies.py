...
# ── .NET: *.csproj `<ItemGroup><PackageReference>` block ─────────────────

def _apply_dotnet(fn_dir: Path, deps: list[str]) -> None:
    """
    Inject NuGet packages into the *.csproj file generated by `func create`.
    Spec format accepted: `PackageName` or `PackageName@1.2.3`.
    Existing references with the same name are replaced (idempotent).
    """
    csproj_files = list(fn_dir.glob("*.csproj"))
    if not csproj_files:
        return
    path = csproj_files[0]
    text = path.read_text(encoding="utf-8")
    ...
    for spec in deps:
        ...
        name = name.strip()
        new_refs[name.lower()] = (
            f'    <PackageReference Include="{xml_escape(name)}" Version="{xml_escape(version.strip() or "*")}" />'
        )

    # Remove existing refs that will be replaced
    lines = text.splitlines()
    kept = []
    for line in lines:
        m = re.search(r'PackageReference Include="([^"]+)"', line, re.IGNORECASE)
        if m and m.group(1).lower() in new_refs:
            continue
        kept.append(line)
    text = "\n".join(kept)

    # Insert new refs before </Project> — create an ItemGroup if needed
    insert_block = "  <ItemGroup>\n" + "\n".join(new_refs.values()) + "\n  </ItemGroup>\n"
    close = "</Project>"
